...
(function(){
  ...
  // Checkout
  var sl={'The Lost Art of Value':'https://buy.stripe.com/dRm28q9oE2POcJz8kc6oo0f','Running From Miracles':'https://shawnedecker.com/#book','VictoryPath':'https://buy.stripe.com/fZufZgeIYgGEfVL6c46oo07','Value Builder':'https://buy.stripe.com/4gM3cu8kAeywaBr43W6oo08','Victory VIP':'https://buy.stripe.com/28E8wO44kbmkdNDbwo6oo09','default':'https://buy.stripe.com/fZufZgeIYgGEfVL6c46oo07'};
...
// === INJECT VALUE BUILDER CARD (3rd tier missing from React bundle) ===
(function() {
  function injectValueBuilder() {
    // Find the pricing grid by locating VictoryPath heading
    var headings = document.querySelectorAll('h3');
    var vpCard = null;
    for (var i = 0; i < headings.length; i++) {
      if (headings[i].textContent.trim() === 'VictoryPath') {
        vpCard = headings[i].closest('[class*="rounded"]');
        break;
      }
    }
    if (!vpCard || document.getElementById('vb-injected')) return;

    var grid = vpCard.parentElement;
    if (!grid) return;

    // Change grid to 3 columns
    grid.className = grid.className.replace('grid-cols-2', 'grid-cols-3');
    if (grid.style) grid.style.maxWidth = '72rem';

    // Check if yearly toggle is active
    var isYearly = false;
    var toggleBtns = document.querySelectorAll('button');
    for (var t = 0; t < toggleBtns.length; t++) {
      if (toggleBtns[t].textContent.indexOf('Yearly') !== -1 || toggleBtns[t].textContent.indexOf('Save') !== -1) {
        var style = window.getComputedStyle(toggleBtns[t]);
    ...
    vbCard.id = 'vb-injected';

    // Make it the "popular" card with shadow and scale
    vbCard.style.boxShadow = '0 25px 50px -12px rgba(0,0,0,0.25)';
    vbCard.style.transform = 'scale(1.05)';
    vbCard.style.zIndex = '10';
    vbCard.style.position = 'relative';

    // Add "Most Popular" badge
    var badge = document.createElement('div');
    badge.style.cssText = 'position:absolute;top:-16px;left:50%;transform:translateX(-50%);background:var(--gold,#f6b638);color:#000;padding:4px 16px;border-radius:9999px;font-size:0.875rem;font-weight:600;display:flex;align-items:center;gap:4px;white-space:nowrap;z-index:11;';
    badge.innerHTML = '\uD83C\uDFC6 Most Popular';
    vbCard.style.paddingTop = '2.5rem';
    vbCard.insertBefore(badge, vbCard.firstChild);

    // Update icon background to solid gold
    var iconDiv = vbCard.querySelector('[class*="rounded-xl"]');
    if (iconDiv && iconDiv.querySelector('svg')) {
      iconDiv.style.background = 'var(--gold,#f6b638)';
    }

    // Update title
    var h3 = vbCard.querySelector('h3');
    if (h3) h3.textContent = 'Value Builder';

    // Update description
    var desc = vbCard.querySelector('p');
    if (desc) desc.textContent = 'Everything you need to build real momentum';

    // Update price
    var priceSpans = vbCard.querySelectorAll('span');
    for (var p = 0; p < priceSpans.length; p++) {
      var txt = priceSpans[p].textContent.trim();
      if (txt.indexOf('$') === 0 && txt.length < 8) {
        if (txt.indexOf('/') === -1) {
          // Main price number
          priceSpans[p].textContent = isYearly ? '$470' : '$47';
        }
      }
      if (txt === '/month' || txt === '/year') {
        priceSpans[p].textContent = isYearly ? '/year' : '/month';
      }
    }
    // Update savings text  
    var savings = vbCard.querySelectorAll('p');
    for (var s = 0; s < savings.length; s++) {
      if (savings[s].textContent.indexOf('save') !== -1 || savings[s].textContent.indexOf('$') !== -1) {
        if (isYearly) {
          savings[s].textContent = '$470/year (save 17%)';
          savings[s].style.color = '#16a34a';
        } else {
          savings[s].textContent = 'or $470/year to save';
          savings[s].style.color = '#16a34a';
        }
        break;
      }
    ...
    var features = [
      {text: 'Everything in VictoryPath', included: true},
      {text: 'Advanced P.I.N.K. tools', included: true},
      {text: 'Priority community access', included: true},
      {text: 'Monthly group coaching', included: true},
      {text: 'Personalized action plans', included: true},
      {text: 'Advanced progress tracking', included: true},
      {text: 'Priority email support', included: true},
      {text: '1-on-1 coaching calls', included: false},
      {text: 'Exclusive VIP events', included: false}
    ];
    var ul = vbCard.querySelector('ul');
    if (ul) {
      ul.innerHTML = '';
      features.forEach(function(f) {
        var li = document.createElement('li');
        li.className = 'flex items-start gap-3' + (f.included ? '' : ' opacity-50');
        li.style.cssText = 'display:flex;align-items:flex-start;gap:0.75rem;margin-bottom:0.75rem;';
        if (!f.included) li.style.opacity = '0.5';
        var iconColor = f.included ? '#22c55e' : '#9ca3af';
        var svgPath = f.included 
          ? 'M5 13l4 4L19 7' 
          : 'M6 18L18 6M6 6l12 12';
        li.innerHTML = '<svg style="width:20px;height:20px;flex-shrink:0;margin-top:2px;" fill="none" stroke="' + iconColor + '" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="' + svgPath + '"/></svg>' +
          '<span style="font-size:0.875rem;color:#52525b;">' + f.text + '</span>';
        ul.appendChild(li);
      });
    }

    // Update CTA button
    var btn = vbCard.querySelector('button');
    if (btn) {
      btn.textContent = 'Join Value Builder \u2192';
      btn.style.background = 'var(--gold,#f6b638)';
      btn.style.color = '#000';
      btn.onclick = function() {
        window.open('https://buy.stripe.com/4gM3cu8kAeywaBr43W6oo08', '_blank');
      };
    }

    // Remove Most Popular badge from VictoryPath if it has one
    var vpBadges = vpCard.querySelectorAll('div');
    for (var b = 0; b < vpBadges.length; b++) {
      if (vpBadges[b].textContent.indexOf('Most Popular') !== -1 && vpBadges[b].style.position === 'absolute') {
        vpBadges[b].remove();
        break;
      }
    }
    // Reset VictoryPath scale
    vpCard.style.boxShadow = '';
    vpCard.style.transform = '';
    vpCard.style.zIndex = '';

    // Insert Value Builder after VictoryPath (middle position)
    vpCard.parentNode.insertBefore(vbCard, vpCard.nextSibling);

    // Also handle toggle clicks to update price
    document.addEventListener('click', function(e) {
      var btn = e.target.closest('button');
      if (!btn) return;
      var text = btn.textContent;
      if (text.indexOf('Monthly') !== -1 || text.indexOf('Yearly') !== -1) {
        setTimeout(function() {
          var vb = document.getElementById('vb-injected');
          if (!vb) return;
          var nowYearly = text.indexOf('Yearly') !== -1;
          var prices = vb.querySelectorAll('span');
          for (var i = 0; i < prices.length; i++) {
            if (prices[i].textContent === '$470' || prices[i].textContent === '$47') {
              prices[i].textContent = nowYearly ? '$470' : '$47';
            }
            if (prices[i].textContent === '/year' || prices[i].textContent === '/month') {
              prices[i].textContent = nowYearly ? '/year' : '/month';
            }
          }
          var savingsP = vb.querySelectorAll('p');
          for (var j = 0; j < savingsP.length; j++) {
            if (savingsP[j].textContent.indexOf('470') !== -1 || savingsP[j].textContent.indexOf('save') !== -1) {
              savingsP[j].textContent = nowYearly ? '$470/year (save 17%)' : 'or $470/year to save';
              break;
            }
          }
        }, 100);
    ...
    console.log('[V2V] Value Builder card injected');
  }

  setTimeout(injectValueBuilder, 2000);
  setTimeout(injectValueBuilder, 4000);
})();
